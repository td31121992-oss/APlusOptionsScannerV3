        ...
        for stock in ranked:
            ...
            safety = self.safety_gate.evaluate(
                ...
                candidate_context={
                    ...
                    "underlying_stop": stop_loss,
                    "underlying_target1": target1,
                    "confidence": confidence,
                    "source": "FULL_OPTION_CHAIN_SCANNER",
                },
            )
            if not safety.allowed:
                logger.info(
                    "Skipping %s: safety gates blocked option plan: %s",
                    symbol,
                    "; ".join(safety.block_reasons),
                )
                continue
