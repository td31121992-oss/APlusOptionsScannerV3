...
_APLUS_SEARCH_CONTROLS = (
    '<input id="stockSearch" type="text" placeholder="Search any stock..." oninput="render()" '
    'style="min-width:190px;background:var(--panel);color:var(--text);border:1px solid var(--line);'
    'border-radius:9px;padding:8px 12px;font-weight:600;outline:none">'
    '<button onclick="document.getElementById(&quot;stockSearch&quot;).value=&quot;&quot;;render()">Clear Search</button>'
)
FNO_MARKET_WATCH_HTML = FNO_MARKET_WATCH_HTML.replace(
    '<span class="sub" id="shown"></span></div>',
    _APLUS_SEARCH_CONTROLS + '<span class="sub" id="shown"></span></div>',
    1,
)
FNO_MARKET_WATCH_HTML = FNO_MARKET_WATCH_HTML.replace(
    "function render(){let a=[...data],",
    "function render(){let a=[...data],q=((document.getElementById(`stockSearch`)||{}).value||``).trim().toUpperCase();"
    "if(q)a=a.filter(x=>String(x.symbol||``).toUpperCase().includes(q)||String(x.sector||``).toUpperCase().includes(q));",
    1,
)
