...
<div class="header"><div><div class="title">APlus Live Trading Terminal</div><div class="sub">Developed by Darpan Bobhate</div><div class="sub">Auto-refreshes every 5 seconds from data/reports</div></div><div style="text-align:right"><div class="sub"><b><span id="trading_day">-</span>, <span id="trading_date">-</span></b></div><div class="sub">Last update: <span id="updated">-</span></div></div></div>
<div class="grid">
...
async function load(){const r=await fetch('/api/snapshot?ts='+Date.now());const d=await r.json();lastData=d;updated.textContent=d.updated_at;trading_date.textContent=d.trading_date;trading_day.textContent=d.trading_day;for(const k of ["total_pnl","open_pnl","closed_pnl"]){let e=document.getElementById(k);e.textContent=fmt(d[k]);e.className="value "+cls(d[k]);}trade_count.textContent=d.trade_count;open_closed.textContent=d.open_count+" / "+d.closed_count;win_rate.textContent=pct(d.win_rate);renderRows(d);}
