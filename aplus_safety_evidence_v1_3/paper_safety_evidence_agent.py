...
def stable_trade_id(t,index):
    for key in ("trade_id","paper_trade_id","id","journal_id"):
        val=str(t.get(key) or "").strip()
        if val:
            return val
    identity="|".join([
        str(t.get("symbol") or ""),
        str(t.get("direction") or t.get("side") or ""),
        str(t.get("entry_time") or t.get("signal_time") or ""),
        str(t.get("option_security_id") or t.get("security_id") or ""),
        str(t.get("strike") or ""),
        str(t.get("expiry") or ""),
        str(index),
    ])
    return "SYN_"+hashlib.sha1(identity.encode("utf-8")).hexdigest()[:16].upper()
...
def compute_state(trades,today):
    ...
    return {
        ...
        "open_premium":round(sum(f(t.get("capital_required",t.get("total_premium",t.get("capital_used")))) for t in opened),2),
        "open_total_risk":round(sum(f(t.get("total_risk",t.get("risk"))) for t in opened),2),
        "source":"paper_trades_latest.json",
    }

def capsule_for(t,tid,market,movement):
    sym=str(t.get("symbol") or "").upper()
    m=market.get(sym,{})
    c=movement.get(sym,{})
    return {
        "captured_at":datetime.now(IST).isoformat(),
        "evidence_version":"V1.3",
        "trade_id":tid,
        "original_trade_id":t.get("trade_id") or t.get("paper_trade_id") or t.get("id"),
        "symbol":sym,
...
def one_cycle():
    ...
    for i,t in enumerate(trades):
        tid=stable_trade_id(t,i)
        if tid.startswith("SYN_"): synthetic+=1
        day=trade_day(t)
        p=CAPS/day/f"{tid}.json"
        atom_write(p,capsule_for(t,tid,market,movement))
        written+=1
    ...
    atom_write(STATUS,{
        ...
        "capsules_written":written,
        "synthetic_ids_used":synthetic,
        "capsules_by_day":by_day,
        "portfolio_state":state,
    })

def main():
    print("="*92)
    print("APlus PAPER Safety + Evidence Agent V1.3")
    print("Synthetic stable IDs enabled when paper report has no trade_id.")
