...
TSRX=re.compile(r"(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})")
...
def run():
    ...
    lines=[]
    if LOG.exists():
        lines=[x for x in LOG.read_text(encoding="utf-8",errors="ignore").splitlines() if day in x]
    cycles=[]; block_rows=[]
    for x in lines:
        m=RX.search(x)
        if m:
            cycles.append({"timestamp":m["ts"],"entry_ready":int(m["e"]),"plans":int(m["p"]),"paper_today":int(m["t"]),"safety_blocked":int(m["b"])})
        if "PAPER_SAFETY_BLOCK" in x:
            tm=TSRX.search(x)
            block_rows.append({"timestamp":tm["ts"] if tm else "","line":x.strip()})

    first_block_ts=min((b["timestamp"] for b in block_rows if b["timestamp"]),default="")
    proof=None
    if first_block_ts:
        proof=next((c for c in cycles if c["timestamp"]>=first_block_ts and c["entry_ready"]>0 and c["safety_blocked"]>0),None)

    caps=ROOT/"data"/"trade_evidence_capsules"/day
    br=ROOT/"data"/"breakout_evidence"/day/"market_watch_1m_ohlcv.csv"
    r={
      "as_of":now.isoformat(),"date":day,
      "latest_cycle":cycles[-1] if cycles else None,
      "paper_safety_blocks":len(block_rows),
      "first_paper_safety_block_timestamp":first_block_ts,
      "runtime_circuit_breaker_proven":bool(proof),
      "proof_cycle":proof,
      "portfolio_state":load(STATE),"evidence_status":load(STATUS),
      "capsules_today":len(list(caps.glob("*.json"))) if caps.exists() else 0,
